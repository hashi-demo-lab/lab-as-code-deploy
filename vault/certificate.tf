...
resource "tls_locally_signed_cert" "vault_cert" {
  ...
  ca_private_key_pem = data.tfe_outputs.ca_cert.values.ca_key_pem
  ca_cert_pem        = data.tfe_outputs.ca_cert.values.ca_cert_pem
...
resource "kubernetes_secret_v1" "vault_tls" {
  ...
  data = {
    ...
    "ca.crt"  = data.tfe_outputs.ca_cert.values.ca_cert_pem
  }
